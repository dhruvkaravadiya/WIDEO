...
export default function VideoPage() {
    ...
    const api = axios.create({
        baseURL: "https://wideo-api.up.railway.app/api",
    ...
    return !videoUrl ? (
        ...
        <div className="mt-14 w-full lg:px-10 flex flex-col lg:flex-row gap-5 sm:px-3 md:px-4">
            <div className="lg:w-8/12 w-full order-1 lg:order-2">
                <div className="mt-4">
                    {videoUrl ? (
                        <video
                            ...
                            preload="auto"
                            poster={video.imgUrl}
                            className="h-50vh rounded-2xl sm:w-full"
                        >
                    ...
                    <div className="flex flex-row gap-4 items-end my-4 px-2">
                        ...
                        <h1 className="text-white text-md font-regular">
                            {video.views} views
                        </h1>
                    </div>
                    ...
                    <div className="flex px-2 flex-col md:flex-row sm:flex-row justify-between gap-4">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="flex sm:items-start items-center md:gap-0 gap-3 flex-row md:flex-col sm:flex-row lg:flex-col">
                                ...
                                <div className="text-sm text-gray-200 font-regular">
                            ...
                            <div
                                className={`flex ${
                                    isMobileScreen ? "justify-end" : ""
                                }`}
                            >
                                <button
                                    onClick={handleSubscription}
                                    className="p-2 px-3 text-black font-bold test-md bg-[#2eacd6] rounded-3xl ms-6"
                                >
                                    {user.subscribers?.includes(currUser.id)
                                        ? "Unsubscribe"
                                        : "Subscribe"}
                                </button>
                            </div>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex flex-row bg-lightblue1 rounded-3xl">
                                <button
                                    onClick={handleLike}
                                    className="flex flex-row rounded-l-3xl hover:bg-lightblue2"
                                >
                                    {video.likes.includes(currUser.id) ? (
                                        <BiSolidLike className="p-2 w-10 h-10 text-slate-400  rounded-l-3xl" />
                                    ) : (
                                        <BiLike className="p-2 w-10 h-10 text-slate-400  rounded-l-3xl" />
                                    )}
                                    <span className="text-slate-400 my-auto me-2 font-md">
                            ...
                            <button
                                onClick={toggleModal}
                                className="flex flex-row justify-center items-center bg-lightblue1 rounded-3xl hover:bg-lightblue2"
                            >
                                <PiShareFat className="w-10 text-slate-400 h-10 p-2 rounded-l-3xl" />
                                <span className="text-slate-400 rounded-r-3xl pe-3 font-semibold text-lg">
                                    Share
                                </span>
                            </button>
                        </div>
                ...
                <div className="my-4 mx-2 whitespace-pre-wrap dark:bg-lightblue1 rounded-xl p-3 shadow-sm">
                    <div className="flex flex-row w-full justify-between">
                        ...
                        <h2
                            className={`text-slate-300 w-fit mb-3 font-regular cursor-pointer`}
            ...
            <div className="lg:w-4/12 w-full p-2 flex flex-col items-start order-1 lg:order-2">
                <h1 className="font-bold text-3xl my-3 text-sky-600 text-archivo">
                    Recommonded
                </h1>
                {videos.map((vid) => (
                    <Link to={"/video/" + vid._id} key={vid._id}>
                        <div className="w-full my-2 h-28 flex flex-row cursor-pointer p-1 rounded-lg">
                            <img
                                ...
                                key={vid._id}
                                alt={vid.title}
                                className="h-26 w-40 rounded-xl"
                            />
                            <div className="ms-4 mt-1 flex flex-col">
                                ...
                                <span className="text-md font-semibold text-slate-500">
                                    {vid.views} views -{" "}
                                    {formatRelativeDate(vid.createdAt)}
                                </span>
                            </div>
                        </div>
                    </Link>
